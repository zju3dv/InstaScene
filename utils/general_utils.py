...
def PILtoTorch(pil_image, resolution, resize_type=Image.LANCZOS, scale=True):
    resized_image_PIL = pil_image.resize(resolution, resize_type)
    resized_image_PIL_np = np.array(resized_image_PIL)
    if resized_image_PIL_np.dtype == np.uint16:
        resized_image_PIL_np = resized_image_PIL_np.astype(np.int16)
    resized_image = torch.from_numpy(resized_image_PIL_np)
    if scale:
...
def create_rotation_matrix_from_direction_vector_batch(direction_vectors):
    ...
    v1 = torch.tensor([1.0, 0.0, 0.0], dtype=torch.float32).to(direction_vectors.device).expand(
        direction_vectors.shape[0], -1).clone()
