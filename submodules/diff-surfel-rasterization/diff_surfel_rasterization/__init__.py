...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(colors_precomp, means3D, scales, rotations, cov3Ds_precomp, radii, extra_attrs, sh,
                              geomBuffer,
                              binningBuffer, imgBuffer)
        return color, radii, depth, extra, gau_related_pixels

    @staticmethod
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_out_extra, grad_gau_related_pixels):

        # Restore necessary values from context
        num_rendered = ctx.num_rendered
        raster_settings = ctx.raster_settings
        colors_precomp, means3D, scales, rotations, cov3Ds_precomp, radii, extra_attrs, sh, geomBuffer, binningBuffer, imgBuffer = ctx.saved_tensors
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args)  # Copy them before they can be corrupted
            try:
                grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_sh, grad_scales, grad_rotations, grad_extra_attrs = _C.rasterize_gaussians_backward(
                    *args)
            except Exception as ex:
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs=None, colors_precomp=None, scales=None, rotations=None,
                cov3D_precomp=None, extra_attrs=None):
