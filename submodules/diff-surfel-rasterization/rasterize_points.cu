...
RasterizeGaussiansCUDA(
    ...
    const torch::Tensor& extra_attrs, // note
    const int attr_degree, // note
	...
	const bool debug) 
{
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations,dL_dextra_attrs);
}
